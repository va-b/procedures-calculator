<template>
    <v-flex>
        <v-card>
            <v-card-title  style="flex-wrap: nowrap;">
              <v-avatar :size="40" class="mr-3">
                  <img :src="parameter.imageUrl"/>
              </v-avatar>
              <div>{{parameter.title}}</div>
            </v-card-title>
            <v-card-text class="pt-0" >
                <v-radio-group :value="$store.getters.SelectedChoiceIdForParameter(parameter.id)"
                               @change="v => $store.commit('CheckChoice', v)"
                               class="mt-0">
                    <v-radio v-for="choice in $store.getters.ChoicesForParameter(parameter.id)"
                             :key="choice.id" :label="choice.title"
                             :value="choice.id"
                    ></v-radio>
                </v-radio-group>
            </v-card-text>
        </v-card>
    </v-flex>
</template>
<script lang="ts">
    import { ParameterDbModel } from "@/model/DataModel";
    import {Component, Prop, Vue} from "vue-property-decorator";

    @Component
    export default class CalcParameter extends Vue {
        @Prop() parameter: ParameterDbModel;
    }


</script>
